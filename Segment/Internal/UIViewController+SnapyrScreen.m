...
+ (UIViewController *)seg_nextRootViewController:(UIViewController *)rootViewController
{
    ...
    if (rootViewController.childViewControllers.count > 0) {
        if ([rootViewController conformsToProtocol:@protocol(SnapyrScreenReporting)] && [rootViewController respondsToSelector:@selector(seg_mainViewController)]) {
...
- (void)seg_viewDidAppear:(BOOL)animated
{
    ...
    if ([top conformsToProtocol:@protocol(SnapyrScreenReporting)] && [top respondsToSelector:@selector(seg_trackScreen:name:)]) {
