...
@implementation SnapyrUserDefaultsStorage

- (instancetype)initWithDefaults:(NSUserDefaults *)defaults namespacePrefix:(NSString *)namespacePrefix crypto:(id<SnapyrCrypto>)crypto
{
    if (self = [super init]) {
        _defaults = defaults;
...
- (id _Nullable)plistForKey:(NSString *)key
{
    ...
    return data ? [SnapyrUtils plistFromData:data] : nil;
}

- (void)setPlist:(id _Nonnull)plist forKey:(NSString *)key
{
    NSData *data = [SnapyrUtils dataFromPlist:plist];
    if (data) {
