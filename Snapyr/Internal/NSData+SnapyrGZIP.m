...
void *_Nullable snapyr_libzOpen()
{
    static void *libz;
...
@implementation NSData (SnapyrGZIP)

- (NSData *)snapyr_gzippedDataWithCompressionLevel:(float)level
{
    if (self.length == 0 || [self snapyr_isGzippedData]) {
        return self;
    }

    void *libz = snapyr_libzOpen();
...
- (NSData *)snapyr_gzippedData
{
    return [self snapyr_gzippedDataWithCompressionLevel:-1.0f];
}

- (BOOL)snapyr_isGzippedData
{
