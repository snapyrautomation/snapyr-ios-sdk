...
#endif
- (NSUInteger)snapyr_beginBackgroundTaskWithName:(nullable NSString *)taskName expirationHandler:(void (^__nullable)(void))handler;
- (void)snapyr_endBackgroundTask:(NSUInteger)identifier;
@end
...
/**
 * Your project's write key from snapyr.com.
 *
 ...
 */
@property (nonatomic, strong, nullable) NSArray<id<SnapyrMiddleware>> *middlewares DEPRECATED_MSG_ATTRIBUTE("Use .sourceMiddleware instead.");

/**
 * Set custom source middleware. Will be run before all integrations
 */
@property (nonatomic, strong, nullable) NSArray<id<SnapyrMiddleware>> *sourceMiddleware;
